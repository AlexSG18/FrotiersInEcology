...
def extract_xml_bbox(xml_file):
    ...
    for obj in objects:
        ...
        p = 1
        if (obj.find('name').text == 'psylla'):
            psylla_bbox.append(np.array([xmin, ymin, xmax, ymax, p]))
        else:
            wasp_bbox.append(np.array([xmin, ymin, xmax, ymax, p]))

    if (len(psylla_bbox) == 0):
...
def plot_gt(img, bboxes):
    ...
    psylla_bbox, wasp_bbox = bboxes

    if psylla_bbox is not None:
        for bbox in psylla_bbox:
            x1, y1, x2, y2, p = [int(b) for b in bbox]
            cv2.rectangle(img, (x1, y1), (x2, y2), (200, 20, 200), 2)
            cv2.putText(img, 'Psylla | GT', (x2, y2), font,
                        0.8, (200, 20, 200), 2, cv2.LINE_AA)

    if wasp_bbox is not None:
        for bbox in wasp_bbox:
            x1, y1, x2, y2, p = [int(b) for b in bbox]
            cv2.rectangle(img, (x1, y1), (x2, y2), (200, 20, 200), 2)
            cv2.putText(img, 'Wasp | GT', (x2, y2), font,
                        0.8, (200, 20, 200), 2, cv2.LINE_AA)
...
checkpoint_file = '/home/alex/Documents/GitHub/FrotiersInEcology/work_dirs/faster_rcnn_r50_fpn_1x_psylla/epoch_20.pth'
